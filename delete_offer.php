...
if (isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
    ...
    $sql_select = "SELECT image_path FROM special_offers WHERE id = ?";
    ...
    $sql_delete = "DELETE FROM special_offers WHERE id = ?";
    if ($stmt_delete = $mysqli->prepare($sql_delete)) {
        $stmt_delete->bind_param("i", $id);
        
        if ($stmt_delete->execute()) {
            // Record deleted successfully. Redirect to manage offer page.
